export const appCopy = {
  schedule: {
    title: {
      en: 'Schedule',
      ko: '일정',
    },
    matrimony: {
      title: {
        en: 'Holy Matrimony',
        ko: '교회예식',
      },
      subtext: {
        en: "Lutheran Church of Our Redeemer Level 3 Sanctuary 30 Duke's Rd, Singapore 268912. Please be seated by 9.30am.",
        ko: '신자들의 구속자 루터교회 3층 성전, 더크스 로드 30, 싱가포르 268912. 오전 9시 30분까지 자리에 앉아 주세요.',
      },
    },
    dinner: {
      title: {
        en: 'Banquet dinner',
        ko: '연회 저녁식사',
      },
      subtext: {
        en: 'One Farrer Hotel Napier, Read & Spottiswoode Level 6 1 Farrer Park Station Road Singapore 217562 The Tea Ceremony for family will begin at 6.30pm. Please be seated by 7.00pm.',
        ko: '원 페러 호텔 나피어, 리드 & 스포티스우드 6층 1 페러 파크 스테이션 로드 싱가포르 217562. 가족을 위한 차 식이 오후 6시 30분에 시작됩니다. 오후 7시까지 자리에 앉아 주세요.',
      },
    },
  },
  common: {
    back: {
      en: 'Back',
      ko: '우리의',
    },
    share: {
      en: 'Share',
      ko: '공유하기',
    },
    cancel: {
      en: 'Cancel',
      ko: '취소',
    },
    maps: {
      en: 'Google maps',
      ko: '구글 맵',
    },
    next: {
      en: 'Next',
      ko: '다음',
    },
    submit: {
      en: 'Submit',
      ko: '제출',
    },
  },
  journey: {
    story: {
      title: {
        header: {
          en: 'Our Story',
          ko: '우리의 이야기',
        },
        subtext: {
          en: 'Once upon a time, our paths crossed. From that moment on, our love story began to unfold',
          ko: '한때 우리의 길이 교차했습니다. 그 순간부터, 우리의 사랑 이야기가 펼쳐지기 시작했습니다',
        },
      },
      howWeMet: {
        header: {
          en: 'How we met',
          ko: '우리의 첫 만남',
        },
        text: {
          en: 'We met in NUS Temasek Hall back in 2019! We were both cast in the same musical production and even though our characters had no scenes or interactions together in the play, we somehow clicked hahaha. Fun fact: when we first met, Eunyu was really hungry so Matt offered her a protein bar he had, saying he didn’t need it and she could have it. She gobbled it up right away but actually the protein bar was super expensive and it was actually a bit heart pain but he now doesn’t regret giving her that protein bar at all ;)',
          ko: '저희는 2019년에 NUS Temasek Hall 에서 만났습니다! 저희는 둘 다 같은 뮤지컬 제작에 캐스팅되었고 비록 저희의 캐릭터들이 연극에서 함께하는 장면은 없었지만, 저희는 친구가 되었습니다. 재미있는 사실: 저희가 처음 만났을 때, 은유는 정말 배가 고파서 매튜는 그녀에게 그가 가지고 있는 단백질 바를 자기는 필요 없다고 하면서 주었습니다. 은유는 즉시 그것을 먹어 치웠지만, 사실 단백질 바는 매우 비쌌고, 매튜는 가슴이 조금 아팠지만, 그는 이제 그녀에게 단백질 바를 준 것을 전혀 후회하지 않습니다 ;)',
        },
      },
      howWeGrown: {
        header: {
          en: 'How we’ve grown',
          ko: '우리의 성장 과정',
        },
        text: {
          en: 'From the wide-eyed university kids to even wider-eyed adults, we’ve learnt so much about ourselves and each other. We learnt (and still learning) how to love each other more each day, through all our idiosyncrasies, cute moments, laughter and tears just as Christ loved us when He gave up His life for us. This self-sacrificial love is still too wide and deep for us to fathom, but we get to learn more about it each day as we live loved by God and live loving one another. We stepped out in faith to move to the same church, pick up new languages and spent time getting to know each others’ friends and family and God has been so faithful to unite our different cultures and worlds together in His grace. He’s blessed us with new family, new friends, new communities that have prayed, supported and helped us become who we are today. ',
          ko: '철없는 대학생들부터 일을 시작하기까지, 저희는 저희 자신과 서로에 대해 많은 것을 배웠습니다. 저희는 재밌는 순간, 귀여운 순간, 웃음과 눈물을 통해 그리스도가 저희를 위해 그의 삶을 포기했을 때 저희를 사랑했던 것처럼 서로를 더 사랑하는 방법을 배웠습니다. 이 자기희생적 사랑은 아직 저희가 헤아리기에는 너무 넓고 깊지만, 하나님의 사랑을 받고 서로 사랑하며 살아가면서 저희는 날마다 그 사랑에 대해 더 많이 알게 됬습니다. 저희는 믿음으로 같은 교회를 다니고 새로운 언어를 배우고 서로의 친구와 가족을 알아가는 데 시간을 보냈습니다. 이 과정중에서 하나님은 은혜로 저희의 다른 문화와 세상을 하나로 묶어 주셨습니다. 하나님는 저희에게 새로운 가족, 새로운 친구, 새로운 공동체를 축복해 주었습니다. 그들의 기도, 지지와 도움으로 저희는 오늘 날의 저희가 되었습니다.',
        },
      },
      themeVerse: {
        title: {
          en: 'Our theme verse',
          ko: '우리 결혼 성경구절',
        },
        text: {
          en: 'A verse that has given us much hope and anchored our relationship from the start is 1 Corinthians 13:7 in the Amplified: “Love bears all things [regardless of what comes], believes all things [looking for the best in each one], hopes all things [remaining steadfast during difficult times], endures all things [without weakening].” In and of ourselves this is truly an impossible task, but this isn’t just an idea of love but Love Himself that lives in us. No matter how different every day may be, whenever we look to the cross and see this very love demonstrated for us, it enables us to love each other in the same way today, tomorrow and for every day of our future. We hope it encourages you too! 😊',
          ko: '처음부터 저희에게 많은 희망을 주고 저희의 관계를 정착시킨 구절은 고린도전서 13장 7절입니다. "사랑은 모든 것을 참으며 모든 것을 믿으며 모든 것을 바라고 모든 것을 견딥니다." 이것은 저희 자신만으로는 정말로 불가능한 일이지만, 하나님께서 먼저 주신 사랑을 기억하며 나아갈것입니다.  하루하루가 다를지라도, 저희는 십자가를 바라보고 저희를 위해 증명된 사랑을 보면서 서로를 사랑하고 싶습니다. 이 구절이 여러분에게도 힘이 되기를 바랍니다! 😊',
        },
      },
    },
    navigation: {
      story: {
        text: {
          en: 'Our story',
          ko: '우리의 이야기',
        },
        subtext: {
          en: "Love's journey revealed",
          ko: '우리의 여정',
        },
      },
      gallery: {
        text: {
          en: 'Gallery',
          ko: '갤러리',
        },
        subtext: {
          en: 'Our cherished moments',
          ko: '우리의 소중한 순간들',
        },
      },
      quiz: {
        text: {
          en: 'Two truths, one lie',
          ko: '두 가지의 진실, 한 가지의 거짓',
        },
        subtext: {
          en: 'How much do you know us?',
          ko: '우리의 대해 얼마나 알고 있나요?',
        },
        innerSubtext: {
          en: 'Guess the lie',
          ko: '거짓말을 맞춰보세요',
        },
        yourScore: {
          en: 'Your score',
          ko: '점수',
        },
      },
    },
  },
  feed: {
    title: {
      header: {
        en: 'Photo Wall',
        ko: '사진 벽',
      },
      subtext: {
        en: 'View and upload cherished moments of the couple from family & friends',
        ko: '가족과 친구들로부터 부부의 소중한 순간을 감상하고 업로드하세요',
      },
    },
    loader: {
      en: [
        'Preparing the bride',
        'Finding the flower girl',
        "Shaving groom's beard",
        'Polishing wedding rings',
        'Frosting wedding cake',
        'Surviving the gatecrash',
        'Memorising vows',
        'Groom learning korean',
      ],
      ko: [
        '신부 준비 중',
        '꽃 소녀 찾기',
        '신랑의 수염 깎기',
        '결혼 반지 닦기',
        '웨딩 케이크 꾸미기',
        '충돌의 관문에서 살아남기',
        '서약 외우기',
        '신랑이 한국어 배우기',
      ],
    },
    upload: {
      title: {
        en: 'Share Your Special Moments With Us',
        ko: '당신이 담은 특별한 순산들을 우리와 공유해주세요',
      },
      subtext: {
        en: "Whether it's a candid shot of the happy couple or a fun-filled moment from the celebration or an unglam photo of the groom, your photos will help create a lasting treasure",
        ko: '행복한 커플의 솔직한 모습이든,  즐거운 축하의 순간이든, 신랑의 어색한 모습이든, 이 사진들을 영원히 간직할 수 있는 보물을 만들기 위해 당신의 도움을 요청합니다',
      },
      placeholder: {
        en: 'Write a caption for the photo',
        ko: '사진의 캡션을 달아주세요',
      },
    },
  },
  messages: {
    title: {
      en: 'From Us, To You',
      ko: '우리로부터, 당신에게',
    },
    notReady: {
      title: { en: 'Hold on awhile longer', ko: '잠시만 기다려 주세요' },
      subtext: {
        en: 'Your personal messages will be coming in shortly',
        ko: '곧 신랑 신부가 준비한 메세지가 도착할 예정입니다',
      },
    },
  },
};
